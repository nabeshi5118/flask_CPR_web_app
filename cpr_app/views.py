...
def upload_file():
 ...
 #テスト用データの処理が汚いからここ直したい
  ...
  if II.load("name") in request.form:
    print("テストデータ")
    Filepath = II.load("filepath_10")
    Filename = II.load("filename_10")
    print(Filepath)
    print(Filename)
...
def progress(filename):
  ...
  exe.estimation_algorithm(app.config['JSON_PROGRESS'],video.flame)
  JP.add({"progress":100})
  print("finish step1")


  JP.add({'message':'データ解析中',"progress":0,"step":2})
  ...
  JP.add({'message':'動画作成中',"progress":0,"step":4})
